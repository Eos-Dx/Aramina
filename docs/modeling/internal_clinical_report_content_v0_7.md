# Aramis Internal Clinical Report Content v0.7

Status: research-draft internal audit contract. It is not for autonomous diagnosis.

Examples use the tracked frozen `0.2.11-beta` artifact only to illustrate
...
final_prediction:
  ...
  score_percentiles:
    ...
    reference_class_training_target_cases: 0.93000
    target_class_training_target_cases: 0.32000
reliability:
  level: high
  reason: at least 3 valid measurements per breast; symmetry refinement applied
symmetry:
  available: true
model_execution:
...
age when available. `reference_class` and `target_class` define the frozen
binary endpoint. `target_class_risk_level` is `high` when `p_cancer >=
decision_threshold.threshold`, otherwise `low`; it is the threshold-derived
decision-support result.
...
or clinical calibration. It does not replace clinician review.
...
training cases, reference-class cases, and target-class cases. The class names
are declared in the same `final_prediction` block. A value of `0.90286` means
that 90.286% of the relevant frozen training scores were lower than the reported
...
full final-model score with SK symmetry terms neutralized. Its
`target_class_risk_level` is calculated with the same shared threshold as the
target breast. The target remains the caller-supplied primary decision-support
result:
...
final_prediction:
  ...
  score_percentiles:
    ...
    reference_class_training_target_cases: 0.67000
    target_class_training_target_cases: 0.25000
reliability:
  level: low
