...
fields are reported separately from risk. If contralateral data is missing, the
SK fields are zero and `symmetry_available = 0`; it is not a model feature.
...
the current candidate uses C=0.1 in both layers
