...
def _():
    ...
    DEFAULT_OUTPUT_JOBLIB_PATH = (
        PRODUCT_DIR / "outputs" / "model_input" / "aramis_all_patients_model_input_v0_1.joblib"
    )
    return (
        AzimuthalIntegration,
        ConstantQRangeTransformer,
...
def _(mo):
    mo.md(
        "\n".join(
            [
                "# Aramis DataFrame All Patients v0.1",
                "",
                "Research draft preprocessing notebook.",
                "",
                "Goal: build patient-safe all-patients model-input DataFrame and save it as joblib.",
                "",
                "H5 branch rule: apply AgBH day, PONI q-range, calibrant-thickness, sample-thickness, and non-NA status filters before GFRM decode. `NORMAL` is mapped to `BENIGN` by the Aramis preprocessing YAML.",
            ]
        )
    )
    return
...
def _(
    ...
    aramis_preprocessing_config_path_input = mo.ui.text(
        ...
        label="Aramis all-patients preprocessing YAML",
    )
...
def _(helpers, mo, snr_df, snr_filter_stats, snr_filtered_df):
    _snr_filtered_profiles_fig = helpers.plot_profiles(
        snr_filtered_df,
        title="All-patients after SNR filter",
        diagnosis_column="product_status_group",
    )
    mo.vstack(
        [
...
def _(helpers, mo, snr_filtered_df, valid_df, validity_stats):
    _valid_profiles_fig = helpers.plot_profiles(
        valid_df,
        title="All-patients after specimen validity filter",
        diagnosis_column="product_status_group",
    )
    mo.vstack(
        [
...
def _(helpers, mo, normalized_df, valid_df):
    _normalized_profiles_fig = helpers.plot_profiles(
        normalized_df,
        title="All-patients after Q 6.7..7.1 normalization",
        diagnosis_column="product_status_group",
    )
    mo.vstack(
        [
...
def _(final_df, helpers, normalized_df):
    final_fig = helpers.plot_profiles(
        final_df,
        title="All-patients final normalized DataFrame",
        diagnosis_column="product_status_group",
    )
