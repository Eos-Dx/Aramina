...
def _():
    ...
    DEFAULT_OUTPUT_JOBLIB_PATH = (
        PRODUCT_DIR / "outputs" / "model_input" / "aramis_biopsy_patients_model_input_v0_1.joblib"
    )
    return (
        AzimuthalIntegration,
        ConstantQRangeTransformer,
...
def _(mo):
    mo.md(
        "\n".join(
            [
                "# Aramis DataFrame Biopsy Patients v0.1",
                "",
                "Research draft preprocessing notebook.",
                "",
                "Goal: build biopsy-patient model-input DataFrame and save it as joblib.",
                "",
                "H5 branch rule: apply AgBH day, PONI q-range, calibrant-thickness, sample-thickness, biopsy-patient, and non-NA status filters before GFRM decode. `NORMAL` is mapped to `BENIGN` by the Aramis preprocessing YAML.",
            ]
        )
    )
    return
...
def _(
    ...
    aramis_preprocessing_config_path_input = mo.ui.text(
        ...
        label="Aramis biopsy-patients preprocessing YAML",
    )
...
def _(
    ...
    h5_filter_plan = helpers.build_branch_h5_filters(
        ...
        branch="one_to_many",
        normal_as_benign=True,
        require_biopsy_patient=True,
    )
...
@app.cell
def _(ProductColumnBuilder, aramis_preprocessing_config, raw_measurement_df):
    _label_config = aramis_preprocessing_config["labels"]["product_column_builder"]
    product_column_builder = ProductColumnBuilder(
        cancer_values=_label_config["cancer_values"],
        benign_values=_label_config["benign_values"],
        normal_values=_label_config["normal_values"],
    )
    ...
    mo.md(
        "\n".join(
            [
                ...
                "`specimenId`-level grouping: `BENIGN/NORMAL -> BENIGN`; `CANCER/ATYPICAL/PRE_CANCEROUS -> CANCER`.",
                "",
...
def _(helpers, mo, snr_df, snr_filter_stats, snr_filtered_df):
    _snr_filtered_profiles_fig = helpers.plot_profiles(
        snr_filtered_df,
        title="Biopsy-patients after SNR filter",
        diagnosis_column="product_status_group",
    )
    mo.vstack(
        [
            ...
            mo.as_html(_snr_filtered_profiles_fig),
        ]
    )
    return
...
def _(helpers, mo, snr_filtered_df, valid_df, validity_stats):
    _valid_profiles_fig = helpers.plot_profiles(
        valid_df,
        title="Biopsy-patients after specimen validity filter",
        diagnosis_column="product_status_group",
    )
    mo.vstack(
        [
            ...
            mo.as_html(_valid_profiles_fig),
        ]
    )
    return
...
def _(helpers, mo, normalized_df, valid_df):
    _normalized_profiles_fig = helpers.plot_profiles(
        normalized_df,
        title="Biopsy-patients after Q 6.7..7.1 normalization",
        diagnosis_column="product_status_group",
    )
    mo.vstack(
        [
            ...
            mo.as_html(_normalized_profiles_fig),
        ]
    )
    return
...
def _(final_df, helpers, normalized_df):
    final_fig = helpers.plot_profiles(
        final_df,
        title="Biopsy-patients final normalized DataFrame",
        diagnosis_column="product_status_group",
    )
            ...
            mo.md(
                "\n".join(
                    [
                        ...
                        "",
                        "Final DataFrame keeps normalized profiles plus selected patient/specimen metadata.",
                        ...
                        "```text",
                        helpers.stage_summary_text(
                            "Final biopsy-patients DataFrame",
                            final_df,
