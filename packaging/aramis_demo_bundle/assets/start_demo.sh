...
case "$(uname -m)" in
  arm64|aarch64)
    API_IMAGE="eosdx/aramis-prediction-api:0.2.12-beta-arm64"
    API_ARCHIVE="${SCRIPT_DIR}/aramis_prediction_api_linux_arm64_0_2_12_beta.tar"
    PLATFORM_IMAGE="eosdx/aramisvisor-demo:0.2.12-beta-arm64"
    PLATFORM_ARCHIVE="${SCRIPT_DIR}/aramisvisor_demo_linux_arm64_0_2_12_beta.tar"
    ;;
  x86_64|amd64)
    API_IMAGE="eosdx/aramis-prediction-api:0.2.12-beta-amd64"
    API_ARCHIVE="${SCRIPT_DIR}/aramis_prediction_api_linux_amd64_0_2_12_beta.tar"
    PLATFORM_IMAGE="eosdx/aramisvisor-demo:0.2.12-beta-amd64"
    PLATFORM_ARCHIVE="${SCRIPT_DIR}/aramisvisor_demo_linux_amd64_0_2_12_beta.tar"
    ;;
...
docker network inspect aramis-demo-network >/dev/null 2>&1 || docker network create aramis-demo-network >/dev/null
docker rm --force aramis-demo aramis-demo-api >/dev/null 2>&1 || true
docker run --detach \
  --name aramis-demo-api \
...
docker run --detach \
  ...
  --volume "${OUTPUT_FOLDER}:/opt/aramisvisor/app/static/reports" \
  "${PLATFORM_IMAGE}" >/dev/null

echo "Aramis browser demonstrator: http://127.0.0.1:${PORT}"
echo "Aramis prediction API: http://127.0.0.1:${API_PORT}/docs"
