"""Fixed Aramis product preprocessing policy layered on XRD YAML validation."""
...
ARAMIS_PREPROCESSING_CONTRACT = "aramis_product_preprocessing_v0_1"
_ROUTES = {
    "aramis_biopsy_patients_model_input": "training",
    "aramis_prediction_patient_model_input": "prediction",
}
_STEP_NAMES = (
    "poni_geometry",
    "select_h5_sessions",
...
def validate_aramis_preprocessing_config(config: dict[str, Any]) -> None:
    """Reject a resolved product YAML that changes the fixed Aramis data route."""
    product = config.get("aramis_preprocessing")
    if not isinstance(product, dict):
        raise ValueError("Aramis preprocessing config requires aramis_preprocessing.")
    name = _nonempty_string(product, "name", "aramis_preprocessing")
    if name not in _ROUTES:
        raise ValueError(f"Unknown Aramis preprocessing route: {name!r}")
    _nonempty_scalar(product, "version", "aramis_preprocessing")
    _nonempty_string(product, "clinical_stage", "aramis_preprocessing")
...
def validate_if_aramis_product_config(config: dict[str, Any]) -> None:
    """Validate product YAMLs while preserving generic XRD pipeline use in tests."""
    if "aramis_preprocessing" in config:
...
def _require_common_policy(config: dict[str, Any]) -> None:
    ...
    if raw_data.get("source") != "gfrm" or raw_data.get("allowed_sources") != ["gfrm"]:
        raise ValueError("Aramis preprocessing requires GFRM as the only raw-data source.")
...
def _require_pipeline_order(config: dict[str, Any]) -> None:
    ...
    if not isinstance(steps, list):
        raise ValueError("Aramis preprocessing requires pipeline.steps.")
    names = [step.get("name") for step in steps if isinstance(step, dict)]
    if names != list(_STEP_NAMES):
        raise ValueError("Aramis preprocessing pipeline steps differ from the approved product order.")
...
def _require_output_columns(config: dict[str, Any], *, route: str) -> None:
    ...
    if not isinstance(columns, list) or not all(isinstance(column, str) for column in columns):
        raise ValueError("Aramis preprocessing metadata.output_columns must be a string list.")
    if len(columns) != len(set(columns)):
        raise ValueError("Aramis preprocessing metadata.output_columns contains duplicates.")
    missing = sorted(_COMMON_OUTPUT_COLUMNS.difference(columns))
    if missing:
        raise ValueError(f"Aramis preprocessing output columns are missing: {missing}")
...
def _mapping(value: dict[str, Any] | None, key: str) -> dict[str, Any]:
    ...
    if not isinstance(child, dict):
        raise ValueError(f"Aramis preprocessing requires mapping {key}.")
    return child
...
def _nonempty_string(value: dict[str, Any] | None, key: str, where: str) -> str:
    ...
    if not isinstance(item, str) or not item.strip():
        raise ValueError(f"Aramis preprocessing requires non-empty {where}.{key}.")
    return item
...
def _nonempty_scalar(value: dict[str, Any] | None, key: str, where: str) -> Any:
    ...
    if isinstance(item, bool | list | dict):
        raise ValueError(f"Aramis preprocessing requires scalar {where}.{key}.")
    return item


def _equal(actual: Any, expected: Any, name: str) -> None:
    if actual != expected:
        raise ValueError(f"Aramis preprocessing requires {name}={expected!r}; got {actual!r}.")
