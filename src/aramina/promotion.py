...
def _validate_source_run(source: Path) -> None:
    ...
    if not isinstance(description, dict) or description.get("output_type") != "aramina_model_description":
        raise ValueError("model_description.yaml is not an Aramina model description.")
