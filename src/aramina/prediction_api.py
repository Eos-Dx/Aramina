"""Local HTTP API for one immutable Aramina prediction artifact."""
...
MODEL_PATH = Path(os.environ.get("ARAMINA_MODEL_PATH", "")).expanduser().resolve()

app = FastAPI(title="Aramina Immutable Model Service", version="0.1")
...
async def predict(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="aramina-api-") as temp:
            temp_path = Path(temp)
