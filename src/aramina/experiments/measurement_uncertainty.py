...
MEASUREMENT_UNCERTAINTY_CONTRACT = "aramina_measurement_uncertainty_v0_1"
PROFILE_SIGMA_ADAPTER = "profile_sigma_gaussian_v0_1"
...
PROFILE_INCLUDED_SOURCES = ("pyfai_poisson_per_bin_sigma",)
...
def run_measurement_uncertainty_from_config(
    ...
    *,
    ...
    profile_summaries, profile_draws = score_profile_sigma_measurement_uncertainty(
        ...
        targets=targets,
        draws=int(config["profile_monte_carlo"]["draws"]),
        seed=int(config["profile_monte_carlo"]["seed"]),
        interval_quantiles=tuple(config["profile_monte_carlo"]["interval_quantiles"]),
    )
    convergence = summarize_profile_monte_carlo_convergence(
        profile_draws,
        checkpoints=tuple(config["profile_monte_carlo"]["convergence_draws"]),
        interval_quantiles=tuple(config["profile_monte_carlo"]["interval_quantiles"]),
    )
    ...
    method_comparison = compare_profile_detector_reference(
        profile_summaries,
        detector_summaries,
    )
        ...
        summary_rows.append(
            {
                ...
                "included_uncertainty_sources": ";".join(PROFILE_INCLUDED_SOURCES),
                "excluded_uncertainty_sources": ";".join(PROFILE_EXCLUDED_SOURCES),
            }
        )
...
def _validate_config(config: dict[str, Any]) -> None:
    ...
    profile_mc = _mapping(config, "profile_monte_carlo")
    _exact_fields(
        profile_mc,
        {
            "draws",
    ...
    _bounded_integer(profile_mc["draws"], 10, 10_000, "profile_monte_carlo.draws")
    _seed(profile_mc["seed"], "profile_monte_carlo.seed")
    quantiles = profile_mc["interval_quantiles"]
    if not isinstance(quantiles, list):
        raise MeasurementUncertaintyError(
            "profile_monte_carlo.interval_quantiles must be a list."
        )
    ...
    if (
        ...
        raise MeasurementUncertaintyError(
            "profile_monte_carlo.convergence_draws must be increasing integers, "
            "start at 10 or more, and end at profile_monte_carlo.draws."
        )
    ...
    if (
        ...
        raise MeasurementUncertaintyError(
            "profile_monte_carlo.convergence_tolerance must be inside (0, 0.1]."
        )
...
def _write_audit_artifacts(
    *,
    ...
    method_comparison.to_csv(
        run_folder / "profile_detector_reference_comparison.csv",
        index=False,
    )
    convergence.to_csv(
        run_folder / "profile_monte_carlo_convergence.csv",
        index=False,
    )
    _write_json(
        ...
        {
            "contract": MEASUREMENT_UNCERTAINTY_CONTRACT,
            "profile_included_uncertainty_sources": list(PROFILE_INCLUDED_SOURCES),
            "profile_excluded_uncertainty_sources": list(PROFILE_EXCLUDED_SOURCES),
            ...
            "profile_detector_reference_cases": int(len(method_comparison)),
            "profile_detector_threshold_crossing_agreement": float(
                method_comparison["threshold_crossing_agreement"].mean()
            ),
            "profile_convergence_checkpoints": sorted(
...
def _log_mlflow_run(
    *,
    ...
    profile_mc = config["profile_monte_carlo"]
    params = {
        "experiment": config["experiment"],
        "profile_monte_carlo": {
            "adapter": PROFILE_SIGMA_ADAPTER,
    ...
    metric_values = {
        ...
        "profile.mean_interval_width": float(
            (
                profile_summaries["p_cancer_high"] - profile_summaries["p_cancer_low"]
            ).mean()
        ),
        "profile.threshold_crossing_patients": float(
            profile_summaries["threshold_crossing"].sum()
        ),
        ...
        "reference.median_profile_to_detector_width_ratio": float(
            method_comparison["profile_to_detector_width_ratio"].median()
        ),
